...
for i = 1 : size(ranges_theta,1)
    ...
    lines_temp = houghlines(redMask,T,R,P,'FillGap',1,'MinLength',window_size(1)/2);
...
if size(centersRedBright,1)==1
    ...
    circ_mask6 = circularMask(window_size/2,min(window_size)/2,window_size) - circ_mask7;
...
if score_white9t < 0.4 && score_white9b < 0.4 && score_red9t > 0.5 && score_red9b > 0.5 && score_red8 < 0.7 && score_white8 > 0.1
    if size(centersRedBright,1)==1
        newROI = [centersRedBright(2)-radiiRedBright centersRedBright(2)+radiiRedBright centersRedBright(1)-radiiRedBright centersRedBright(1)+radiiRedBright];
        result = 'stop--circle';
    elseif score_red1 > 0.73 && abs(window_size(1)-mean(window_size))<0.05*mean(window_size) && score_white8 > 0.2 && score_white1 > 0.2 && score_white1 < 0.3
         result = 'stop--color';
    end
end

...
for ww = 1 : length(directory)
    ...
    for k = 1 : length(theFiles)
        ...
        %figure();imshow(mask3);
        CC = bwconncomp(mask3);
        region = regionprops(CC,'Area','BoundingBox', 'Centroid', 'MajorAxisLength','MinorAxisLength','Orientation','Perimeter','Eccentricity');
        ...
        a(ww).feature(k).Perim = cat(1,region.Perimeter);
        a(ww).feature(k).Eccentricity = cat(1,region.Eccentricity);
        ...
        e(ww).all_Perim = cat(1,a(ww).feature.Perim);
        e(ww).Eccentricity = cat(1,a(ww).feature.Eccentricity);
        ...
        C(ww).feature(k).Perim = cat(1,region_convex.Perimeter);
        C(ww).feature(k).Eccentricity = cat(1,region_convex.Eccentricity);
        ...
        O(ww).all_Perim = cat(1,C(ww).feature.Perim);
        O(ww).Eccentricity = cat(1,C(ww).feature.Eccentricity);
...
for zz=1:length(directory)
    ...
    max_Eccentricity = max(e(zz).Eccentricity);
    min_Eccentricity = min(e(zz).Eccentricity);
    average_Eccentricity = mean(e(zz).Eccentricity);
    max_Perim = max(e(zz).all_Perim);
    min_Perim = min(e(zz).all_Perim);
    average_Perim = mean(e(zz).all_Perim);
    max_area = max(e(zz).all_area);
    min_area = min(e(zz).all_area);
    average_area = mean(e(zz).all_area);
    
    AAAA(zz).max_Perim = max_Perim;
    AAAA(zz).min_Perim = min_Perim;
%     AAAA(zz).average_Perim = average_Perim; 
    AAAA(zz).max_area = max_area;
    AAAA(zz).min_area = min_area;
%     AAAA(zz).averageArea = average_area;
    AAAA(zz).max_Eccentricity = max_Eccentricity;
    AAAA(zz).min_Eccentricity = min_Eccentricity;
%     AAAA(zz).average_Eccentricity = average_Eccentricity;
    %hold on
    %     scatter(zz,average_area,15,'filled');
    %     scatter(x,e(zz).all_area,15,'filled');
    C_max_Perim = max(O(zz).all_Perim);
    C_min_Perim = min(O(zz).all_Perim);
    C_average_Perim = mean(O(zz).all_Perim);
    C_max_area = max(O(zz).all_area);
    C_min_area = min(O(zz).all_area);
    C_average_area = mean(O(zz).all_area);
    C_max_Eccentricity = max(O(zz).Eccentricity);
    C_min_Eccentricity = min(O(zz).Eccentricity);
    C_average_Eccentricity = mean(O(zz).Eccentricity);
    
    CCCC(zz).C_max_area = C_max_area;
    CCCC(zz).C_min_area = C_min_area;
%     CCCC(zz).C_average_area = C_average_area;
    CCCC(zz).C_max_Perim = C_max_Perim;
    CCCC(zz).C_min_Perim = C_min_Perim;
%     CCCC(zz).C_average_Perim = C_average_Perim; 
    CCCC(zz).C_max_Eccentricity = C_max_Eccentricity;
    CCCC(zz).C_min_Eccentricity = C_min_Eccentricity;
%     CCCC(zz).C_average_Eccentricity = C_average_Eccentricity; 

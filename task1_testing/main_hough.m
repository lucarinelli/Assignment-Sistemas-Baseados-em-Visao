...
for k = 1 : 1 %length(theFiles)
    ...
    %% testing hough
        ...
        % plot found peak lines
       lines = houghlines(just_red,theta,rho,P,'FillGap',5,'MinLength',7);
       
       % plotting the found lines
       figure(), imshow(original), hold on
        max_len = 100;
        for f = 1:length(lines)
           xy = [lines(f).point1; lines(f).point2];
